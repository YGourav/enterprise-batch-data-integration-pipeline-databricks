...
# MAGIC %md
# MAGIC ##Initialize notebook widgets
...
base_path = f's3://fmcgproject-childcompany-data/{data_source}'
...
#1. Keeps only rows with order_qty present
df_orders = df_orders.filter(F.col('order_qty').isNotNull())

#2. Keep only numerics in customer_id, keep 999999 for invalid ones
...
display(df_orders.limit(15))

# COMMAND ----------

# MAGIC %md
# MAGIC ### Join with Products table to get Products_code
...
# MAGIC %md
# MAGIC ### In Child the data is on daily level aggregation, but in parent the data is aggregated monthly

# COMMAND ----------

df_child = spark.sql(f"SELECT date, product_code, customer_code, sold_quantity FROM {gold_table}")
df_child.show(10, False)
